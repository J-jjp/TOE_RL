...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        """
        # 检查接触力是否超过阈值，如果任何一个维度的范数大于1，则认为需要重置
        self.reset_buf = torch.any(torch.norm(self.contact_forces[:, self.termination_contact_indices, :], dim=-1) > 1., dim=1)
        
        # 检查机器人的滚转角（roll）、俯仰角（pitch）和偏航角（yaw）是否超过阈值
        # 如果俯仰角绝对值大于1.0或滚转角绝对值大于0.8，则认为需要重置
        self.reset_buf |= torch.logical_or(torch.abs(self.rpy[:,1])>1.0, torch.abs(self.rpy[:,0])>0.8)
        
        # 检查是否超时，如果当前回合长度大于最大允许的回合长度，则认为超时
        self.time_out_buf = self.episode_length_buf > self.max_episode_length # 超时不会有终止奖励
    ...
    def _compute_torques(self, actions):
        ...
        """
        #pd controller
        actions = self.last_actions * 0.2 + actions * 0.8
    ...
    def _init_buffers(self):
        ...
        self.foot_positions = self.rigid_body_state.view(self.num_envs, self.num_bodies, 13)[:, self.feet_indices,
                              0:3]
        self.gait = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
...
# ——————————————————————————————————————————————————————————————————————————————————————--
    ...
    def _step_contact_targets(self):
        if self.cfg.env.observe_gait_commands:
                # if (self.commands[env_ids, 0] > 0) & (self.commands[env_ids, 1] > 0):
            frequencies = self.gait.fill_(3.0)
            phases = self.gait.fill_(0.5)
            durations = phases
            # print("durations.size:", durations.size())
            offsets = self.gait.fill_(0.0)
            bounds = offsets
            ...
            # print("foot_indices.size:", self.foot_indices.shape)
...
# ————————————————————————————————————————————————————————————————————————————————————-----
    ...
    def _reward_stand_still(self):
        ...
        # return torch.exp(-torch.sum(torch.abs(self.dof_pos - self.default_dof_pos), dim=1) * (torch.norm(self.commands[:, :2], dim=1) < 0.1)/self.cfg.rewards.stand_still_sigma)
    ...
    # Need to filter the contacts because the contact reporting of PhysX is unreliable on meshes
        ...
        return torch.sum(torch.square(self.dof_pos[:,[2,5,8,11]] - (-2.0)) * (~contact_filt),dim=1)
    def _reward_hip_pos(self):
        # return torch.sum(torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_dof_pos[:, [0, 3, 6, 9]]), dim=1)
        return (1-torch.abs(self.commands[:,1])) *  torch.sum(torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_dof_pos[:, [0, 3, 6, 9]]), dim=1)
    def _reward_raibert_heuristic(self):
        ...
        # self.gait[:, 4]=2.0
        frequencies = self.gait.fill_(2.0)
